  ...
  if (!pipelines.length && !tasks.length) {
    return (
      <div className="view-page">
        ...
        <EmptyState icon={<GitBranch size={30} />} title="Your project monitor is ready" description="Create the structure yourself or ask Codex to inspect the enrolled folder and submit a draft for review." action={<div className="button-row"><Link to={`/projects/${project.id}/outline?action=new-pipeline`} className="button button-primary button-md"><GitBranch size={17} />Create pipeline</Link><Link to={`/projects/${project.id}/outline?action=new-task`} className="button button-secondary button-md"><ListChecks size={17} />Create task</Link><Button variant="secondary" onClick={copyPrompt}>Copy Codex initialization prompt</Button></div>} />
      </div>
    )
  }
  return (
